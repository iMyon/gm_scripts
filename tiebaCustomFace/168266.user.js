...
var addFaces = function()
{
	...
	{
		baiduFace1[$(this).attr('data-index')] = $(this).find('span').html();
		baiduFace1.length++;
	});
	...
	for (var index=0;index<faces.length;index++)
	{
		...
		if(face.position==0)
			selector.find('li.s_tab_btn[data-index="'+(parseInt(face.position)+1)+'"]').before('<li id="smilyTab'
				+index+'" class="s_tab_btn customSmily"><span class="s_tab_btnbg" data-index="'+face.tabName+'">'
				+face.tabName+'</span></li>');
		else
			selector.find('li.s_tab_btn[data-index="'+(parseInt(face.position))+'"]').after('<li id="smilyTab'
				+index+'" class="s_tab_btn customSmily"><span class="s_tab_btnbg" data-index="'+face.tabName+'">'
				+face.tabName+'</span></li>');
		tabOnClick(index);
	}
...
var addBlacklistDiv=function()
{
	...
	for(var i=0;i<baiduFace1.length;i++)
		html=html+'<span class="blistElement" index="'+i+'">'+baiduFace1[i]+'</span>';
	html+='<br>';
	...
	cf.addEventListener('click',function(){$('#blist').remove();});
}
...
{
	selector.find('DIV.s_layer_tab.j_tab').after('<div id="setting">'
												+'<div id="aditFaceMenu">编辑</div>'	//增加编辑按钮
												+'<div id="addFaceMenu">增加</div>'		//增加按钮
												+'<div id="deleteFaceMenu">删除</div>'	//删除按钮
												+'<div id="tab_MoveLeft">左移</div>'
												+'<div id="tab_MoveRight">右移</div>'
												+'<div id="getProfile">导出</div>'
												+'<div id="loadProfile">导入</div>'
												+'<div id="showFace">隐藏</div>'
												+'</div>');
		...
		for(var i=0;i<row;i++)
		{
			...
			for(var j=0;j<face.colCount;j++)
			{
				...
				if(currentFaceSrc)
				{
					...
					tbody[index]+='<td class="s_face j_emotion" index="'+cindex+'" border="1" data-surl="'+currentFaceSrc
					...
					tbody[index]+='<a class="img imgclasses" onclick="onClickImg(\''
						+currentFaceSrc+'\')" style="background:none;"><img class="faceImg" src="'+currentFaceSrc+'" style="max-height:'+face.faceWidth_Height+'px;max-width:'+face.faceWidth_Height+'px;"></img></a>';
					tbody[index]+='</td>';
				}
			}
			tbody[index]+='</tr>';
...
var addNewFace = function()
{
	...
	var config_1 = function()
	{
    	...
    	selector.find('li.s_tab_btn[data-index="1"]').before('<li id="smilyTab'
				+cindex+'" class="s_tab_btn customSmily"><span class="s_tab_btnbg">'
				+selector.find('#tabName').val()+'</span></li>');
    	selector.find('.selected').removeClass('selected');
...
//设置栏
//悬浮层
var floatdiv = '<div id="maxwindow"><div id="Face_float">'
	+'<textarea id="Face_floatedit">'
	+'</textarea></div>'
	+'<div id="tabSetting"><span id="d_tabName">标签名:<input id="tabName" type="text" onkeydown="if(event.keyCode==13) return false;">'
	+'</span><span id="d_colCount">每行表情:<input id="colCount" type="text" onkeydown="if(event.keyCode==13) return false;"></span></div>'
	+'<div id="config"><span id="Face_yesc" class="fff">保存</span><span id="Face_noc" class="fff" >取消</span></div></div>';
...
var tab_MoveLeft = function()
{
	var cindex = selector.find('.selected').parent().attr('id').replace("smilyTab","");
  	cindex = parseInt(cindex);
	//console.log(1);
	if($(css+' #smilyTab'+cindex).prev().length!=0&&cindex>0
		&&$(css+' #smilyTab'+cindex).prev().attr('id')!="smilyTab0")
	{
	...
	(function c()
	{
		if(document.querySelector('.bdimgshare-p-icon.bdshare-i-mshare'))
		{
			...
			for(var i=1;i<faces.length;i++)
				content=content+'<a class="bdimgshare-p-icon bdshare-i-mshare ccccc" onclick="return false;" href="#" data-btype="'
				+i+'">'+faces[i].tabName+'</a>';
			var html='<h6>添加到</h6>'
			+'<div class="bdimgshare-layer-list">'
			+content
			+'</div>';
...
var showAddFaceDiv=function()
{
	if(!document.querySelector('#addFaceDiv'))
	{
		...
		for(var i=1;i<faces.length;i++)
		{
			cdiv=cdiv+'<a class="addface" onclick="return false;" href="#" data-btype="'
				+i+'">'+faces[i].tabName+'</a>';
		}
...
function normal_Listener()
{
	...
	if(a)
	{
		...
		{
			css = '.edui-container';
			if(document.querySelector('.edui-container li.s_tab_btn[data-index="1"]')
				&&(!document.querySelector('.edui-container #smilyTab0')))
			{
				$('.edui-container li.s_tab_btn[data-index="1"]').click();
